...
#
# Modified by Earl Sammons for use in the Honeynet Project Honeywall CDROM
#
...
#EWS Name: %{realname}%{inlinetext}
#EWS %{?_with_inline:%define Name: %{realname}-inline }
Name: %{realname}
Version: 2.6.1.5
Epoch: 1
#EWS Release: %{release}
Release: %{release}.hw.2
...
#EWS <NEW>
Source99: snort_inline.conf
#</NEW>
BuildRoot: %{_tmppath}/%{name}-%{version}-%{release}-root-%(%{__id_u} -n)

#EWS Packager: Official Snort.org %{for_distro}
Packager: Honeynet Project
Vendor: %{vendor}
BuildRequires: autoconf automake pcre-devel
#EWS Conflicts: %{conflicts}
#EWS<NEW>
Provides: snort-%{version}
Provides: snort-inline-%{version}
...
%if %{inline}
#EWS BuildRequires: libnet
BuildRequires: libnet iptables-devel pcre-devel libpcap-devel
...
This is a customized RPM for the honeynet Project "roo" Honeywall.  It
is designed to be built with 'rpmbuild --with inline' and it generates
two identical "Inline" aware binaries in %{_sbindir} named "snort-plain"
and "snort-inline".  It satisfies Walleye <=1.1.21 requirement for "snort",
"inline awareness" requirement, obsoletes (removes) unecessary snort-mysql,
and requires (forces the install of) the newly creates "Community-Rules" RPM
for "IDS" snort (which no longer supplies rules).

You have been warned... Use at your own risk outside of the honeywall.
...
#EWS<NEW>
	%__mkdir_p -m 0755 $RPM_BUILD_ROOT/%{_sysconfdir}/snort_inline
#EWS</NEW>
...
#EWS<NEW>
	%__install -p -m 0755 %{name}-plain $RPM_BUILD_ROOT%{_sbindir}/%{name}-inline
#EWS</NEW>
...
#EWS	%__install -p -m 0755 rpm/snortd $RPM_BUILD_ROOT%{_initrddir}
	%__install -p -m 0644 rpm/snort.sysconfig $RPM_BUILD_ROOT/%{_sysconfdir}/sysconfig/%{realname}
#EWS	%__install -p -m 0644 rpm/snort.logrotate $RPM_BUILD_ROOT/%{_sysconfdir}/logrotate.d/snort
#EWS	%__install -p -m 0644 etc/reference.config etc/classification.config \
#EWS		etc/unicode.map etc/gen-msg.map etc/sid-msg.map \
#EWS		etc/threshold.conf etc/snort.conf etc/generators \
#EWS<NEW>
	%{__install} -p -m 0644 etc/snort.conf $RPM_BUILD_ROOT%{_sysconfdir}/snort/snort.conf
#EWS<NEW>
...
#EWS <NEW>
# Install snort_inline.conf
	%{__install} -p -m 0644 %{SOURCE99} $RPM_BUILD_ROOT%{_sysconfdir}/snort_inline/snort_inline.conf
#</NEW>
...
#EWS<NEW>
# Turn unified alerting on...
%__sed -i 's#^.*output alert_unified: filename.*$#output alert_unified: filename /var/log/snort/snort_unified, limit 128#' etc/snort.conf
# Enable full/fast logging
%__sed -i 's#^.*output <name_of_plugin>:.*$#output alert_full: snort_full\noutput alert_fast: snort_fast#' etc/snort.conf
# Set search-method to acs (2.6.1.1)
#%__sed -i 's;^#.*config detection: search-method.*$;config detection: search-method acs;' etc/snort.conf
#EWS</NEW>
...
#	/usr/sbin/useradd -M -d %{_var}/log/snort -s %{noShell} -c "Snort" -g snort snort 2>/dev/null || true
	...
	/usr/sbin/useradd -M -d %{_var}/log/snort -s %{noShell} -c "Snort" -g snort snort
fi

#EWS <NEW>
# Clean out any exising links/files that might remain
%__rm -f %{_sbindir}/snort* 2> /dev/null || true
#EWS </NEW>
...
#EWS if [ -L %{_sbindir}/snort ] || [ ! -e %{_sbindir}/snort ] ; then \
#EWS	%__rm -f %{_sbindir}/snort; %__ln_s %{_sbindir}/%{name}-plain %{_sbindir}/snort; fi

# We should restart it to activate the new binary if it was upgraded
#EWS %{_initrddir}/snortd condrestart 1>/dev/null 2>/dev/null

# Don't do all this stuff if we are upgrading
if [ $1 -eq 1 ] ; then
	%__chown -R snort.snort %{_var}/log/snort
#EWS	/sbin/chkconfig --add snortd
fi
#If upgrade, make sure we fix these in existing snort.conf
if [ $1 -eq 2 ]; then
# Fix dynamic-preproc paths
	%__sed -i 's,^dynamicpreprocessor.*$,dynamicpreprocessor directory %{_libdir}/%{realname}-%{version}_dynamicpreprocessor,' /etc/snort/snort.conf
# Fix dynamic-engine paths
	%__sed -i 's,^dynamicengine.*$,dynamicengine %{_libdir}/%{realname}-%{version}_dynamicengine/libsf_engine.so,' /etc/snort/snort.conf
fi

%preun
#EWS if [ $1 = 0 ] ; then
	# We get errors about not running, but we don't care
#EWS	%{_initrddir}/snortd stop 2>/dev/null 1>/dev/null
#EWS	/sbin/chkconfig --del snortd
#EWSfi
...
if [ $1 -eq 0 ] ; then
#EWS	if [ -L %{_sbindir}/snort ]; then
#EWS		%__rm -f %{_sbindir}/snort
#EWS <NEW>
...
#EWS %attr(0644,root,root) %config(noreplace) %{_sysconfdir}/snort/classification.config
#EWS %attr(0644,root,root) %config(noreplace) %{_sysconfdir}/snort/reference.config
#EWS %attr(0644,root,root) %config(noreplace) %{_sysconfdir}/snort/threshold.conf
#EWS %attr(0644,root,root) %config(noreplace) %{_sysconfdir}/snort/*.map
#EWS %attr(0644,root,root) %config(noreplace) %{_sysconfdir}/snort/generators
...
#EWS <NEW>
%attr(0644,root,root) %config(noreplace) %{_sysconfdir}/snort_inline/snort_inline.conf
#EWS </NEW>
%attr(0644,root,root) %config(noreplace) %{_sysconfdir}/sysconfig/snort
#EWS %attr(0755,root,root) %config(noreplace) %{_initrddir}/snortd
...
###############################################################################
# Honeynet Changelog
# * Mon Mar 19 2007 Earl Sammons <esammons@hush.com> 2.6.1.3-hw.4
# - Updated Buildrequires for inline
# - Re-building against new kernel-2.6.20.x headers and libpcap-devel
# -  Not absolutely sure above is necessary, but doing it to be safe
